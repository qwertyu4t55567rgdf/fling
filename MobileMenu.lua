...
local function script3l()
	...
	btnfly.Parent = gui
	btnfly.Name = "Flying"
	btnfly.BackgroundTransparency = 0
	btnfly.BackgroundColor3 = Color3.new(0, 0, 0)
	btnfly.Size = UDim2.new(0, 40, 0, 40)
	btnfly.Position = UDim2.new(0.62, 0, 0.537, 0)
	btnfly.TextScaled = true
	btnfly.Text = "Fly"
	btnfly.TextColor3 = Color3.new(1, 1, 1)
	btnfly.Visible = true
	
	local ucflybtn = Instance.new("UICorner")
	
	ucflybtn.Parent = btnfly
	ucflybtn.CornerRadius = UDim.new(0, 8)
	...
	run.RenderStepped:Connect(function()
		if fly == true and plr.Character.Humanoid.MoveDirection ~= Vector3.new(0, 0, 0) then
			local cameralook = camera.CFrame.LookVector
			
			local movedirection = Vector3.new(0, 0, 0) + cameralook
			
			plr.Character.HumanoidRootPart.Velocity = movedirection * 30
		end
	end)
end
